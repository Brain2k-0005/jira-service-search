              ...
              <CardContent>
                ...
                <div className="space-y-4">
                  {selectedDepartment && selectedCategory ? (
                    ...
                    // Display all subcategories from all departments when no specific category is selected
                    <>
                          ...
                          <div key={subcategory.id} className="p-4 border rounded-md border-border bg-card/50 hover:bg-card transition-colors">
                            ...
                            <div className="flex justify-between items-start">
                              <div>
                                ...
                                <p className="text-sm text-muted-foreground">{subcategory.description}</p>
                                <p className="text-xs text-muted-foreground mt-1">
                                  Department: {department.name} | Category: {category.name} | 
                                  Contains {subcategory.services.length} {subcategory.services.length === 1 ? 'service' : 'services'}
                                </p>
                              </div>
                              <div className="flex gap-2">
          ...
          <TabsContent value="services" className="mt-6">
            <Card>
              ...
              <CardContent>
                <div className="space-y-4 mb-6">
                  <div>
                    <Label htmlFor="select-department-service" className="block mb-2">Select Department (Optional)</Label>
                    <Select 
                      value={selectedDepartment} 
                      onValueChange={(value) => {
                        setSelectedDepartment(value);
                        // Reset other selections if department changes
                        setSelectedCategory('');
                        setSelectedSubcategory('');
                      }}
                    >
                      <SelectTrigger id="select-department-service" className="w-full">
                        <SelectValue placeholder="All departments" />
                      </SelectTrigger>
                      <SelectContent>
                        <SelectItem value="">All departments</SelectItem>
                  ...
                  {selectedDepartment && (
                    <div>
                      <Label htmlFor="select-category-service" className="block mb-2">Select Category (Optional)</Label>
                      <Select 
                        value={selectedCategory} 
                        onValueChange={(value) => {
                          setSelectedCategory(value);
                      ...
                      >
                        <SelectTrigger id="select-category-service" className="w-full">
                          <SelectValue placeholder="All categories" />
                        </SelectTrigger>
                        <SelectContent>
                          <SelectItem value="">All categories</SelectItem>
                  ...
                  {selectedDepartment && selectedCategory && (
                    <div>
                      <Label htmlFor="select-subcategory" className="block mb-2">Select Subcategory (Optional)</Label>
                      <Select 
                ...
                <div className="space-y-4">
                  {selectedDepartment && selectedCategory && selectedSubcategory ? (
                    ...
                    // Display all services from all departments when no specific subcategory is selected
                    <>
                      <h3 className="font-medium text-foreground">All Services</h3>
                      
                      {getAllServices().length > 0 ? (
                        getAllServices().map(({ service, subcategory, category, department }) => (
                          <div key={service.id} className="p-4 border rounded-md border-border bg-card/50 hover:bg-card transition-colors">
                            <Breadcrumb className="mb-2 text-xs">
                              <BreadcrumbList>
                                <BreadcrumbItem>
                            ...
                            <div className="flex justify-between items-start">
                              <div>
                                ...
                                <p className="text-sm text-muted-foreground">{service.description}</p>
                                <p className="text-xs text-muted-foreground mt-1">
                                  Department: {department.name} | Category: {category.name} | Subcategory: {subcategory.name}
                                </p>
                                {(service.link || service.contactEmail) && (
                                  <div className="mt-2 space-y-1">
                                    {service.link && (
                              ...
                              <div className="flex gap-2">
                                ...
                                <Button 
                                  ...
                                  size="sm" 
                                  className="text-destructive border-destructive/20 hover:bg-destructive/10"
                                  onClick={() => deleteService(department.id, category.id, subcategory.id, service.id)}
                                >
                                  <Trash className="h-4 w-4 mr-1" />
                                  Delete
                                </Button>
                        ...
                        <p className="text-center py-4 text-muted-foreground">
                          {serviceFilter
                            ? "No services match your filter"
                            : "No services found. Create your first one below."}
                        </p>
                      )}
                    </>
                  )}
                  ...
                  <div className="mt-6 pt-6 border-t border-border">
                    ...
                    <form className="space-y-4" onSubmit={(e) => { e.preventDefault(); addService(); }}>
                      ...
                      <div className="grid gap-2">
                        <Label htmlFor="service-link">Request Link (Optional)</Label>
                        <Input 
                          id="service-link" 
